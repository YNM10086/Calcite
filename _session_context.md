...
- 📌 **当前数据规模（2026-09-25 实测复核，M3 收尾）**：
  **246 条轨迹 / 286,019 个点 / 264 个停留点 / 37 个热点 / 33 个孤立点**
  （264 = 37 个热点吃掉 231 + 33 个孤立；接口实测 `scannedTracks=246 / scannedStays=264 / ΣvisitCount=231`，
  与 2026-09-21 那组**完全一致** —— 中途一度变成 265/232，是下面那条遗留轨迹造成的，清理后回归）
  - ⚠️ **track `20081203151206` 的 id 现在是 `652`**（历次：**39 → 249 → 652**）——
    这条被验收脚本删过**两次**（数据管理的"删除后相似度必须变"、`verify-data-edit-api.py` 的设计性删除），
    每次重新导入都拿新 id，**数据本身一致**（279 点 / 3611.3 m / 2539 s）。
    ⭐ **教训：脚本里绝不要写死轨迹 id** —— `verify-similarity-api.py` 被**同一个坑绊了两次**
    （39 时代红一次、249 时代又红一次，都是 `count(*)` 为 0 → **除以零**）；
    现在改为按 **`external_id`（文件内容 SHA-256，删掉重导也不变）** 解析当前 id，从根上断掉漂移
  ...
  - ✅ **遗留轨迹 id 650「verify-new-41c272」（305 点 / external_id `8899c415…`）已清理（2026-09-25）** ——
    它是 `verify-data-edit-api.py` 带提权运行留下的验收遗留物（删完要清理时回收站导出失败，轨迹留在了库里）。
    沙箱内删不掉（`DELETE` 撞回收站导出 → **500 且轨迹原样保留**），**主控提权重启后端后走 API 删掉**，
    回收站副本 `20260925-172429_verify-new-41c272_305点.geojson`（19 KB）。
    清理后 **247 → 246 条、286,324 → 286,019 点**（差额正好 305）
...
- ✅ **用户点名的 Word 交付物已交**（2026-09-21）——
  ...
  **M3 的那份（2026-09-25 M3 圈选笔记）见本文件 M3 小节末尾。**
...
4. **⚠️ `verify-data-edit-api.py` 与 `check-data-edit.py` 是「数据变更操作」，不是只读验收** ——
   **现象**：它们**按设计真的会删掉一条轨迹**（删前先导出到回收站），跑完库里的轨迹数会变。
   M3 收尾时实际发生过：`verify-data-edit-api.py` 删掉了 `249 / 20081203151206`（279 点），
   库一度变成 **245 / 285,740**；后来从源文件
   `D:\Calcite-note\GPX-Data\Geolife Trajectories 1.3\Data\000\Trajectory\20081203151206.plt`
   重新导入（**新 id = 652**）才回到 246 / 286,019。
   **根因**：这两个脚本验的是"删除/新增后缓存是否失效"，所以**必须真删真加**才有效力 ——
   这是刻意的，不是 bug。
   **解法 / 纪律**：① 跑之前先知道**自己会动数据**（别把它当普通回归随手跑）；
   ② 跑完**按脚本末尾打印的提示恢复**（从 `D:\Calcite-note\GPX-Data\...\<时间戳>.plt` 重新导入，
   **id 会变**，所以写死 id 的脚本要一并改 —— 见本条上面的教训）；
