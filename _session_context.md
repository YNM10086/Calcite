...
- ⭐ **用户当时的疑问**：以为"自己的数据没形成热点所以没显示"。
  ...
  50 米内停 5 分钟的时刻）—— 这半对；② 但**热点显示了 3 个**，只是全部来自 GeoLife，
...
- **M2 第一、二阶段已完成**（停留点识别 → 停留热点），前端「停留点 / 热点」切换可用
- **下一步 = M2 第三阶段「网格密度」**（原叫「热点区域分析」）：接口用 `/api/analysis/density`
  与 `hotspot` 区分；对 `track_point` 做 `ST_SnapToGrid` 密度统计
- **再之后 = 第四阶段「轨迹相似度」**（判断两条轨迹是不是走的同一条路）
- 开工前值得先定的一件事：**要不要趁这时把停留点入库 `stay_point`** ——
  触发条件是轨迹数 > **500 条** 或 热点接口 > **2 秒**；当前 25 条轨迹现算够快，所以还没入库
- 本地未推送的提交：从 `ce7a902`(文档同步) 起的 M2 全部提交（停留点 → 面板改版 → 热点阶段，最新 `6220047`）；
  按推送节奏，**攒到 M2 整体完成再推**
