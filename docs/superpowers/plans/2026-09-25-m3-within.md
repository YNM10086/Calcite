...
class RegionGeometryTest {
    ...
    void Point加bufferM成功() {
        var r = RegionGeometry.parse(json("{\"type\":\"Point\",\"coordinates\":[116.32,40.00]}")   // ⚠️ 不能用单行文本块：Java 要求 """ 后必须换行, 500.0, MAXV, MAXB);
    ...
    void Point没有bufferM要拒绝() {
        var e = assertThrows(IllegalArgumentException.class,
                () -> RegionGeometry.parse(json("{\"type\":\"Point\",\"coordinates\":[116.32,40.00]}")   // ⚠️ 不能用单行文本块：Java 要求 """ 后必须换行, null, MAXV, MAXB));
    ...
    void bufferM为零或负数或NaN都要拒绝() {
        for (Double bad : new Double[]{0.0, -1.0, Double.NaN, Double.POSITIVE_INFINITY}) {
            assertThrows(IllegalArgumentException.class,
                    () -> RegionGeometry.parse(json("{\"type\":\"Point\",\"coordinates\":[116.32,40.00]}")   // ⚠️ 不能用单行文本块：Java 要求 """ 后必须换行, bad, MAXV, MAXB),
                    "bufferM=" + bad + " 应该被拒绝");
        }
    }
    ...
    void bufferM超过上限要拒绝() {
        assertThrows(IllegalArgumentException.class,
                () -> RegionGeometry.parse(json("{\"type\":\"Point\",\"coordinates\":[116.32,40.00]}")   // ⚠️ 不能用单行文本块：Java 要求 """ 后必须换行, MAXB + 1, MAXV, MAXB));
    }
...
- [ ] **Step 1: 给 `TrackRepository` 追加四个方法**
...
class WithinServiceTest {
    ...
    private static List<Object[]> prepared() {
        return List.of(new Object[]{Boolean.TRUE,
    ...
    void 没有区域内点数的轨迹算0而不是报错() {
        ...
        when(tracks.findWithinSummariesByIds(any())).thenReturn(new ArrayList<>(List.of(summary(1L, 10.0))));
