...
public final class RegionGeometry {
    ...
    public static Region parse(JsonNode geometry, Double bufferM, int maxVertices, double maxBufferM) {
        ...
        // ── 多边形 / 多多边形 ───────────────────────────────────────────
        // 统一成 List<环>，每个环是 List<double[lon,lat]>
        List<List<double[]>> rings = new ArrayList<>();
        if ("Polygon".equals(type)) {
            collectRings(coords, rings);
        } else {
            for (JsonNode polygon : coords) {
                collectRings(polygon, rings);
            }
        }
        if (rings.isEmpty()) {
            throw new IllegalArgumentException("geometry 里没有任何环");
        }
        ...
        if ("Polygon".equals(type)) {
            sb.append("POLYGON(");
            appendRings(sb, rings);
            sb.append(')');
        } else {
            sb.append("MULTIPOLYGON(");
            // 每个 polygon 用一层额外括号；这里只有一个环组的常见情况也成立
    ...
    private static void collectRings(JsonNode coords, List<List<double[]>> out) {
        if (!coords.isArray() || coords.isEmpty()) {
            throw new IllegalArgumentException("多边形的 coordinates 必须是非空数组");
        }
        // Polygon 是 [[[x,y],...]]，MultiPolygon 的一层是 [[[x,y],...]]
        if (coords.get(0).isArray() && !coords.get(0).isEmpty() && coords.get(0).get(0).isArray()) {
