...
> ⚠️ **本节已按修复轮 7 修订（2026-09-25，用户实测「多边形选框只能是三角形」）**：
> 下面 Step 的代码示例里凡是要**留住**鼠标位置的地方 —— `rectStart = m.position`、
> `drawScreenPoints = [...drawScreenPoints, m.position]` —— 一律改成**存副本**
> （`screenCopy(p) = new Cartesian2(p.x, p.y)`）。
> **原因**：Cesium 的鼠标事件对象是**模块级单例**（`CesiumUnminified/index.js:237297-237320`：
> `mouseUpEvent` / `mouseClickEvent` 每次被 `Cartesian2.clone` 写进**同一个对象**），
> handler 每次收到的 `m.position` 是同一个实例 ⇒ 存引用则数组里每一项都等于
> "最后一次点击的位置" ⇒ `screenDistance(本次点击, drawScreenPoints[0])` 恒为 0
> ⇒ 第 4 次点击被判成"点回起点" ⇒ **自由多边形永远只能是三角形**
> （规格 2.7 第二处更正 / 11.13）。实测：修复前点 4 个顶点发出的请求体外环只有 4 个坐标。
>
> ⚠️ **替身同步改（否则夹具永远抓不到这类缺陷）**：`.tmp/cesium-stub.mjs` 新增
> `clickAt / moveTo / downAt / upAt / dblClickAt`（**复用同一个对象**，复刻 Cesium 的单例语义），
> `.tmp/check-runtime-task9.mjs` 的所有事件载荷改用它，并新增两条 ⭐ 断言
> （"第 4 个点离起点 566px 不闭合"、"四点闭合的载荷 = 4 个顶点"）。
> 旧写法让测试自己造 `{ position: { x, y } }` 字面量（每次都是新对象）——
> 和 enableRotate 那次同一个病根：**替身的形状必须与运行时一致**。
...
> ⚠️ **本节已按修复轮 7 补两条判据（2026-09-25，用户实测「多边形只能是三角形」）**：
> 多边形那段只断言"查出了数字"**是不够的**（三角形照样能查出数字）。必须再加：
> ① 点完 4 个顶点时 `POST /api/analysis/within` 的**计数不变**（第 4 点离起点 44px ≫ `CLOSE_PX`=12
> 绝不能闭合）；② 闭合后请求体的**去重顶点数 = 4**。
> 为此 `on_request` 要把请求体记进 `within_bodies`，并加 `within_ring_distinct(body)` 读外环去重顶点数。
> 这两条是端到端硬证据 —— 缺陷版本会在①就红（第 4 次点击直接发请求，外环 4 个坐标 = 三角形）。
...
2. 新增「M3 第一阶段 · 空间范围查询（2026-09-25 完成）」小节，写清：
   接口、三种形状、`region` 回显、缓冲区 303→7.9 ms 的实测、平面/球面 13.4 km 的已知限制、
