...
- 轨迹相似度比对：⚠️ **原计划的 `ST_FrechetDistance`（PostGIS 内置）实测不可用，已改为「双向重合度取小」**。
  **该项已于 2026-09-17 提前完成（作为 M2 第四阶段，见附录 B.1）**：接口 `GET /api/analysis/similarity?trackId=&toleranceM=&limit=`，
  度量是 `min(正向重合度%, 反向重合度%)`，容差默认 50 米。详见 `docs/superpowers/specs/2026-09-17-m2-similarity-design.md`
...
| **M2 停留点：现算不存库** | ✅ 已定（2026-09-14）—— `stay_point` 表**保持空着**。理由：一条轨迹几千个点现算只要几毫秒，而且参数一改结果立刻跟着变，不用管缓存失效。等 M3 要做「跨轨迹查询」（某个区域被停留过几次）时再入库才划算。详见 `2026-09-14-m2-stay-point-design.md` |
| **M2 停留热点（跨轨迹聚类）** | ✅ 已完成（2026-09-17）—— 并查集单链聚类，`GET /api/analysis/hotspots`。**数据扩充后**（246 条轨迹 / 264 个停留点）→ **37 个热点 + 33 个孤立点**。停留点仍**不入库** —— 改成按 `trackId` 内存缓存（轨迹导入后不可变，缓存永不失效），热态 **1.9 秒**（缓存前 4.9 秒）。若将来还要压，触发入库的条件见设计文档 10.2 |
| **M2 网格密度（原「热点区域分析」）** | ✅ 已完成（2026-09-17）—— `GET /api/analysis/density`，按视野返回网格密度。**实现上没用 `ST_SnapToGrid`，而是等价的 `round(ST_X(geom)/cell)`**（快约 2 倍、不落盘；两者除格子边界点外等价，默认档 0.002 上严格相同）。支持时段筛选与两个口径，前端由 Cesium 相机事件驱动、从固定档位阶梯挑格边长。详见 `docs/superpowers/specs/2026-09-17-m2-density-design.md` |
| **M2 轨迹相似度** | ✅ 已完成（2026-09-17）—— `GET /api/analysis/similarity?trackId=&toleranceM=&limit=`，度量是**双向重合度取小** `min(fwd%, rev%)`（容差默认 **50 米**）。实测 track 20 与 **197 条**轨迹比对，第一名 track 39 = **91.1%**。首次约 **2 秒**，之后按 `(trackId, toleranceM)` 缓存瞬间返回。**取小是关键**：track 6（1.3 km）完全落在 track 18（20.7 km）上时单向 100%、双向只有 **34.4%**，这个"被包含"必须被压下去。**`ST_FrechetDistance` 实测不可用**（单位是度不是米、跨两城没有单一投影带能罩住、转 geography 无此函数、对 GPS 跳点极度敏感），详见 `docs/superpowers/specs/2026-09-17-m2-similarity-design.md` |

> ✅ **M2 四个阶段全部完成（2026-09-17）**：停留点识别 → 停留热点 → 网格密度 → 轨迹相似度。
> 至此"从播放升级为分析"的里程碑达成，M2 收官。
