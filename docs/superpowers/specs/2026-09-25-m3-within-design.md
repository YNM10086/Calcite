...
### 2.4 ⚠️ 平面几何 vs 球面距离：在这 3 条轨迹上最大差 13.4 km
...
|---|---|---|
| **蝴蝶结（自交）多边形** | **false** | ⚠️ **不报错**，照样返回一个数字（实测 **237 条**）。⚠️ 2026-09-25 更正：原写"比它的外接矩形（232 条）还多"是**拿两个不同区域的数字作比较**——那个蝴蝶结**自己的**外接矩形同样是 237，而 232 是 2.2 节那个更小的北京框。**论据改掉，结论不变**：非法几何给出的数字与任何有意义的区域都不对应，且**不报错** |
...
**平面解释与球面解释在"含超长边的轨迹"上会分歧，最大 13.4 km。**
...
| **缓冲区** | 单击地图选中心 | 单击即查（用当前半径） | 随即查询；**改完半径可再点「查询」用同一中心重查** |
...
> ⚠️ **6.3 的一处更正（2026-09-25，最终全分支审查发现）**：缓冲区原设计写的是
> "单击中心（显示**待定圆点**）→ 点「查询」"。实现改成了"**点中心即以当前半径查询**"，
> 而"待定圆点"并不存在 —— 这个偏离**当时没有记录**（本项目其它偏离都补了裁定与规格更正，唯独这条漏了）。
>
> 更要紧的是它带出一个真缺陷：`RegionDrawer` 的查询按钮原本 `:disabled="mode !== 'buffer'"`，
> 而点中心后 `drawingMode` 立刻回到 `idle` → **按钮永久置灰**，于是"同一中心改半径重查"这条
> **该按钮存在的唯一理由**在最需要它的时刻不可用；失败路径又把 `bufferCenter` 一起清掉，
> **400 之后连绕行办法都没有**。
>
> **处置**：① 加 `hasCenter` prop，按钮改为 `busy || !hasCenter`（有中心就能查）；
> ② 失败路径不再清 `bufferCenter`（只在用户主动"清除/切档"时清）；
> ③ 提示语改成与实现一致的表述。**教训**：把"两步交互"改成"一步"时，
> 要连带检查**第二步的入口**是否还存在。
### 6.4 地图上画什么
...
- ✅ **实测结果（2026-09-25，`.tmp/verify-within-api.py`）**：最终 **97 项通过 0 项失败**（三轮：96 → 98 → 97，最后一轮把一条哨兵降级为信息输出）。北京大框（limit=500）接口总耗时三次 **437 / 203 / 559 ms**，**落在 2.5 预期的 200~600 ms 区间内**（另两轮 628 / 249 / 597 与 410 / 290 / 599），远低于 1000 ms 红线 → **不动用 2.5 的两条候选改法**。同一脚本还实测：全排除时间窗（from=2020-01-01）→ `total=0` 且 `pointCount=0`（4 ms 快路径生效）；正常窗（2008-11）→ `pointCount=42752`；缓冲区圆的面积 3,132,102 m² vs π·1000² 相对差 **0.30%**。
...
| 项 | 说明 |
|---|---|
| **平面 vs 球面的分歧** | 只影响 3 条含超长边的轨迹（121/161/166），最大 **13.4 km**（id 161；121 是 11.7 km、166 是 12.2 km）。见 3.4，**有意不修**，用测试钉住 |
| **非法几何一律 400，不自动修复** | 自交 / 面积为 0 的多边形会被挡住，**不用 `ST_MakeValid` 猜用户意图**（理由见 11.11）。用户偶尔要重画一次 |
| **极快甩动绘制后地球会轻微跳一下** | 实测：Playwright 合成拖拽（8 步几十毫秒）后视野中心位移约 **20% 跨度**；而**人类速度**拖拽（每步 200ms）位移**精确为 0**。原因是 Cesium 的输入聚合器在快速手势下累积了速度，松手恢复 `enableRotate` 后它被应用。**不修**（要碰 Cesium 内部；且真实用户看不到），验收脚本里把"包围盒位移"降级为信息输出。见 9.5 |
...
2. **"静默"正是要消灭的东西。** 实测（2.9）PostGIS 对自交多边形**不报错**，直接给出一个数字（237 条）
