...
- ✅ **实测结果（2026-09-25，.tmp/verify-within-api.py，96 项通过 0 项失败）**：北京大框（limit=500）接口总耗时三次 **437 / 203 / 559 ms**，**落在 2.5 预期的 200~600 ms 区间内**，远低于 1000 ms 红线 →**不动用 2.5 的两条候选改法**。同一脚本还实测：全排除时间窗（from=2020-01-01）→ 	otal=0 且 pointCount=0（4 ms 快路径生效），正常窗（2008-11）→ pointCount=42752。
