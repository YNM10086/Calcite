...
> ⚠️ **2.7 的第二处更正（2026-09-25，用户实测「多边形选框只能是三角形」后定位）**：
> Cesium 的**鼠标事件对象是模块级单例** —— handler 每次收到的 `m.position` / `m.endPosition`
> 是**同一个 `Cartesian2` 实例**，只是里面的 x/y 被改写。
> 依据（`frontend/node_modules/cesium/Build/CesiumUnminified/index.js`）：
> `:237297-237302` 定义模块级的 `mouseUpEvent` / `mouseClickEvent`（`mouseDownEvent` /
> `mouseMoveEvent` / `mouseDblClickEvent` 同样是模块级），`:237320` 的
> `Cartesian2.clone(position, mouseClickEvent.position)` 把新位置**写进同一个对象**再交给 handler。
>
> 后果：**任何"把 `m.position` 存进数组或变量"的写法都会退化成"只剩最后一次点击的位置"**。
> `drawScreenPoints` 里每一项都是同一个引用 ⇒ `screenDistance(本次点击, drawScreenPoints[0])`
> **恒为 0** ⇒ 第 4 次点击（哪怕离起点 566 px）被判成"点回起点"而闭合
> ⇒ **自由多边形在真实交互里永远只能是三角形**（"鼠标回到起点高亮"也退化成比对最后一个顶点）。
>
> 实测（探针 `.tmp/probe-polygon-triangle.py`，判据 = `POST /api/analysis/within` 的请求体）：
>
...
>
> 修法：`screenCopy()` = `new Cartesian2(p.x, p.y)`，`rectStart` 与 `drawScreenPoints` **只存副本**。
>
> ⭐ **教训（第二层，比第一次更细）**：2.7 第一次更正的教训是"类型声明证明不了属性挂在哪个对象上"；
> 这一次是"**签名对不等于语义对**"—— `m.position` 的类型完全正确、文档也写着 `Cartesian2`，
> 但"每次都是同一个实例"是**实现细节，`.d.ts` 里看不出来**。
> 因此替身也必须复刻这一点（`.tmp/cesium-stub.mjs` 的 `clickAt/moveTo/downAt/upAt/dblClickAt`
> 复用同一个对象）：旧替身放行测试自己写的 `{ position: { x, y } }` 字面量，
> 于是"存引用"这类缺陷在夹具里**永远不会红** —— 和 enableRotate 那次是同一个病根。
> 判据也要挑对：旧浏览器验收只断言"多边形查出了数字"，**三角形照样能通过**；
> 现在钉住"点完 4 个顶点还不发请求"与"请求体去重顶点数 = 4"（见 9.5）。
...
> ⚠️ **6.3 的第二处更正（2026-09-25，用户实测「多边形选框只能是三角形」）**：
> 上表"多边形：单击逐个加点"的描述是对的，**是实现错了** —— 第 4 次点击被误判成"点回起点"，
> 于是真实交互里**只能画出三角形**（根因：Cesium 鼠标事件对象是模块级单例，组件存了引用，
> 见 2.7 第二处更正）。现已修复；回归判据见 9.5 的两条 ⭐ 断言。
>
...
- 多边形：点 4 个点 + 双击 → 同上；⭐ **并且必须钉住"点完 4 个顶点还没闭合（请求数不变）"
  与"请求体去重顶点数 = 4（四边形，不是三角形）"** —— 只断言"多边形查出了数字"是不够的：
  三角形照样能查出数字、照样能让那条断言通过（2026-09-25 的实测缺陷就是这么溜过去的，见 2.7）
...
### 11.13 ⭐ 为什么屏幕坐标要经 `screenCopy()` 存副本（而不是直接存 `m.position`）

**决定**：`CesiumGlobe.vue` 里凡是要"留住"的鼠标位置（`rectStart`、`drawScreenPoints`）
一律经 `screenCopy(p) = new Cartesian2(p.x, p.y)` 复制后再存；**绝不存事件对象本身**。

**为什么**：Cesium 的鼠标事件对象是**模块级单例**（2.7 第二处更正），
handler 每次拿到的 `m.position` 是同一个实例。存引用 ⇒ 数组里每一项都等于
"最后一次点击的位置" ⇒ "点回起点"判断恒真 ⇒ **多边形只能画出三角形**。

**与 11.11 的对照**：11.11 是"**要不要替用户猜意图**"（决定：不猜，非法几何 400）；
这一条是"**要不要相信手里这个对象**"（决定：不相信，用完立刻快照）。
两者的共同点：**不要把"看起来正确"当成"语义正确"**。

**若不这么做**：缺陷不会报错、不会抛异常、`stat-tracks` 还是能查出数字 ——
它只在**人真正用手画多边形**时才暴露（自动化脚本写字面量，永远测不出来）。
这也解释了它为什么能穿过 M3 的四层验收：**判据问的是"有没有结果"，而不是"结果对不对"**。
