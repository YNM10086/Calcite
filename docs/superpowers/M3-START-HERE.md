...
> **M3 已经全部做完了。** 这份文件是**开工前**的导读，保留下来作为"当时怎么规划的"的记录。
> 要了解 M3 实际做了什么，请看下面两份：

| 角色 | 文件 |
|---|---|
| **设计文档（权威）** | `docs/superpowers/specs/2026-09-25-m3-within-design.md` |
| **实施计划** | `docs/superpowers/plans/2026-09-25-m3-within.md` |

**两项交付的结论：**

| M3 交付项 | 状态 |
|---|---|
| **空间范围查询** `POST /api/analysis/within`（拉框 / 自由多边形 / 缓冲区；前端第五档「圈选」） | ✅ 完成 |
...
> —— 第 4 次点击被误判成"点回起点"闭合。根因是 Cesium 的鼠标事件对象是**模块级单例**，
> 组件存了引用（详见设计文档 **2.7 第二处更正** / **11.13**）。
> 修法：`screenCopy()` 存副本；同时给替身补上单例语义、给浏览器验收补两条硬判据
> （within 从 42 → **44** 项）。这条缺陷**穿过了 M3 的四层验收**，因为它只在"人真正用手画"时暴露：
> 判据问的是"有没有结果"，三角形照样有结果 —— 记在 `_session_context.md` 的踩坑记录第 5 条。

> ⚠️ 分支 `feat/m3-within` 当时只做了本地提交；**合并回 main 与推送由主控在最终审查后处理**。
> （2026-09-25 已快进合并回 `main`；**推送仍未做 —— 用户选择"先不推"**。）
