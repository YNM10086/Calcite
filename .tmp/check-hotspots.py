...
⚠️ 期望值一律**当场问接口要**，脚本里不许再出现"热点只有几个 / 轨迹有多少条"这类写死的数量。
    2026-09-17 导入数据后（轨迹总数翻了一个量级、热点数也随之变多），本文里原来写死的
...
# 改动它会同时改界面行为，因此不会像"当前有几个热点"那样被导数据打破。
HOTSPOT_LIST_CAP = 50
...
def fetch_hotspots():
    ...
    2026-09-17 又导入了一批 GeoLife 之后，热点数也跟着涨了一截，所有写死数量的断言
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # 现在热点多了很多，fitBounds 要装下全部，视野一拉远、红热点在屏幕上反而更近
        # （实测只剩两三百像素宽）—— 判据失效，但功能是好的（红像素上万，画得好好的）。
        #
        ...
        # 原来写死"29 分钟"（那是早期少量热点时的数据）；现在按接口算出"谁该第一"，
